...
**Таргетинг устройств** — те же три способа, что и в
[Окнах обслуживания](maintenance-windows.md#на-какие-устройства-действует-окно--три-способа-таргетинга)
(подсети / профили / список конкретных устройств, можно комбинировать) — **но с
противоположным поведением пустого селектора**: если не заполнить ни одного из трёх
полей — правило сработает **для всех устройств**. Форма явно показывает это пояснение
при пустом селекторе, чтобы не перепутать с логикой окон обслуживания.
...
**Получатели** — отметьте, каких пользователей системы уведомлять. Уведомление уйдёт на
**все включённые каналы** каждого выбранного пользователя (если у человека настроены и
email, и Telegram — придёт в оба, если оба включены).
