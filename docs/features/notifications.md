...
- **При активации тревоги** / **При закрытии тревоги** — два независимых переключателя:
  ...
  на то, и на другое.
- **Повторные уведомления** — явно включаемая политика с задержкой первого повтора,
  интервалом, максимальным числом повторов и уровнями escalation/получателями. Без этой
  настройки повторов нет; обычные activation/clear-уведомления не меняются.

Повтор отправляется только пока тревога активна и не подтверждена. Acknowledgement, recovery,
ручное закрытие и активный snooze останавливают повторную доставку. Flapping также подавляет
activation/recovery, чтобы частые переходы не создавали шторм уведомлений.
