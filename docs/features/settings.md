...
Раздел «Настройки» — пять групп параметров работы самой системы: как часто что-то
опрашивается, сколько хранить историю, параметры проверки доступности, подключения к
базам данных и настройки встроенного ИИ-ассистента.
...
Учётные данные провайдера ИИ — общие на всю систему, их использует каждый пользователь
при работе с чатом ассистента (см. [ИИ-ассистент](ai-assistant.md)). Что именно ассистент
может прочитать или изменить в конкретном чате, определяется правами того пользователя,
который с ним общается, а не этими настройками.

- **Включён** — общий переключатель чата на весь экземпляр.
- **Провайдер** — один из трёх:
  - **GigaChat** — `Authorization key` (единый ключ из личного кабинета GigaChat Studio,
    сам является `base64(Client ID:Client Secret)` и разбирается автоматически) + тариф
    `scope`, который должен совпадать с реально подключённым в кабинете.
  - **OpenRouter** — `API key` (бесплатный можно завести на openrouter.ai без привязки
    карты) + `model` (например `openai/gpt-4o-mini`, или `:free`-суффикс — учтите, что не
    все `:free`-модели поддерживают вызов инструментов).
  - **Self-hosted (vLLM/Ollama/...)** — полный адрес chat-completions эндпоинта
    (`http://localhost:11434/v1/chat/completions` для Ollama, `…/v1/chat/completions` для
    vLLM) + опциональный `API key` (многие self-hosted серверы работают без авторизации).

Ключи и секреты хранятся зашифрованными (требуется настроенный `DISCOVERY_SECRET_KEY` на
сервере) и никогда не приходят обратно с сервера — поля всегда пустые при открытии;
пустое поле = «сохранить текущий секрет», новый текст = «заменить его».
