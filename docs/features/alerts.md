...
| Статус | «Активна» / «Закрыта» (закрыта вручную — рядом имя закрывшего пользователя) / «Утеряна» (см. ниже). Дополнительный бейдж «подавлена» — тревога признана следствием другой, ещё активной тревоги (см. [Групповые тревоги](group-alerts.md)); клик по бейджу открывает карточку причинной тревоги (устройство, имя, уровень, статус), не нужно искать её вручную по списку. |
...
- **Период автообновления** — от отключено до раза в секунду (по умолчанию — раз в
  секунду).
