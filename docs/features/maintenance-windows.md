...
### На какие устройства действует окно — способы таргетинга (2)

Можно комбинировать оба способа одновременно — устройство попадает под окно, если
...
- **Подсети (CIDR-диапазоны)** — устройство должно попадать хотя бы в одну из указанных
  подсетей (IP сети + маска). Точечное наведение на одно конкретное устройство —
  подсеть `/32` с его адресом.
- **Профили** (через запятую) — устройство должно быть на одном из перечисленных
  профилей мониторинга.

**Устройства (ID)** — третье поле (точный список UUID устройств) существовало в форме
до 2026-09-20 и было из неё убрано по прямому запросу пользователя: сырые UUID негде
взять оператору, а точечное наведение на одно устройство уже закрывается подсетью `/32`.
Сам механизм не выпилен — окно, созданное через REST API или сохранённое раньше, по-прежнему
учитывает свой список конкретных устройств (`device_ids`), просто из формы он больше не
редактируется.

**Важно:** если не заполнить ни одного из полей — окно не будет подавлять тревоги
