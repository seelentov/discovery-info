...
остальное, что появилось через дискаверинг или было добавлено вручную.
...
[«Дискаверинг»](discovery-configs.md), проходят полную проверку сразу же, как часть
...
- **Топологические соседи (иконка слоёв)** — открывает окно со списком соседей этого
  устройства по каждому настроенному правилу [Топологии](topology.md): если правил
  несколько, показаны все сразу, не нужно проверять каждое по отдельности; правило без
  единого соседа у этого устройства тоже показывается явно, не пропускается молча.
...
## Разовое обнаружение сервисов

Кнопка «Обнаружить сервисы» запускает для указанного IP один bounded-набор проверок:
SNMP, SSH, HTTP, HTTPS и TCP. Результат каждой проверки содержит ok, задержку и текст
ошибки. На каждую пробу действует таймаут, общий запуск ограничен 64 пробами и 30 секундами.

При успешном результате устройство разрешается идемпотентно по IP, а назначается профиль с
наивысшим приоритетом среди успешных проверок. Дальше обычный reconciliation продолжает
подхватывать мониторинг. Если не прошла ни одна проверка, запись устройства не создаётся.

## Операторские действия и backup

Для включённого устройства с правом Devices:Write доступны четыре фиксированных действия:

- **Test** — ping и разрешённые профильные service checks;
- **Poll now** — немедленный SNMP-опрос метрик;
- **Rediscover** — повторное SNMP-enrichment с передачей проверки идентичности в Dedup;
- **Backup** — профильная read-only SSH-команда получения конфигурации.

Каждое действие становится job с видимым статусом и причиной ошибки. Одновременно для
устройства выполняется не более одного такого job, повтор недавнего завершённого действия
ограничен. Backup требует профиль, SSH-креды и закреплённый host key; произвольную команду
оператор передать не может.

История backup доступна отдельно от общей истории jobs. Сырые данные хранятся зашифрованно,
API/UI показывают только redacted/normalized конфигурацию, а сравнение двух успешных версий
возвращает bounded unified diff. SNMP backup в текущей версии не поддерживается.
...
дискаверинга — оно появится в списке сразу, без ожидания SNMP-опроса.
...
— как через эту форму, так и через дискаверинг (лимит общий, а не отдельный для каждого
