...
**По умолчанию каждое изменяющее действие требует вашего явного подтверждения.** Если вы просите
...
## Режим без пошагового подтверждения для одного разговора

Над перепиской есть переключатель, который выключает пошаговое подтверждение для
**текущего разговора**. Пока он включён, поддерживаемые изменяющие действия выполняются
сразу, без карточки «Подтвердить/Отклонить». Опасные или необратимые действия по-прежнему
остановятся на подтверждении. Модель доступа не меняется — каждое действие проверяет
права того, кто ведёт разговор, в момент выполнения; в журнале такие записи отдельно
помечены как выполненные автоматически. Настройка хранится только у текущего разговора,
не является глобальной настройкой и по умолчанию выключена. Включайте её только там, где
вы доверяете сформулированной задаче.
