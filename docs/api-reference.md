...
- **Ошибки — не JSON.** Тело ошибки — обычный текст (`Content-Type: text/plain`), не
  ...
  структуре.
- Списочные эндпоинты (устройства, тревоги, профили, дискaверинг, задачи очередей, логи,
...
`snmp_auth` — тот же формат, что и `credentials` внутри подсети дискaверинга (раздел
«Дискaверинг» ниже): `{"kind": "community", "version": "v1"|"v2c", "community": "..."}`
...
`des`/`des3`/AES-варианты для `priv`) перечислены в форме дискaверинга — см.
[Дискaверинг](features/discovery-configs.md#snmp-доступ).
...
## Дискaверинг (домен `discovery_configs`)
...
пробного периода (1 конфигурация) — см. [Дискaверинг](features/discovery-configs.md).
